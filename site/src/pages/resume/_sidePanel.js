...
function SideBarHeader({ title, children }) {
  return (
    <div className="mt-5 text-center">
      <h2 className="text-purple-900 print:text-black uppercase">
        {title || children}
      </h2>
    </div>
  )
...
function SidePanel({ resumeData, siteMetadata, fullContent }) {
  const {
    siteUrl,
  ...
  return (
    <div className={`card min-w-[210px] pb-2`}>
      ...
      <div className="mt-5 text-sm print:text-black">
        <section className="text-center">
          <div className="inline-block my-auto">
            <LocationMarkerIcon className="inline-block -mt-[5px] h-5 w-5" />
            <span className="ml-1 inline-block">{resumeData.location}</span>
          </div>
        </section>
            ...
            <li>
              <Link href={linkedin} forceExternal>
                <div className="flex items-center gap-1 ml-[2px]">
                  ...
                  /in/paulo-henrique-menin
                </div>
              </Link>
            </li>
            <li>
              <Link href={github} forceExternal>
                <div className="flex items-center gap-1 ml-[2px]">
                  <div className="h-4 w-4 fill-black">
                    <svg
                      role="img"
        ...
        <section className="text-center">
          <SideBarHeader>PERSONAL SKILLS</SideBarHeader>
          <ul>
        ...
        <section className="text-center">
          ...
          <ul>
            {resumeData.techinicalSkills.map((skill, index) => {
              return <li key={index}>{skill}</li>
            })}
          </ul>
        </section>
        ...
        {fullContent && (
          <section className="text-center">
            <SideBarHeader>TECHNICAL STACK</SideBarHeader>
            <ul>
              {resumeData.techinicalStack.map((skill, index) => {
                return <li key={index}>{skill}</li>
              })}
            </ul>
          </section>
